...
import kr.hhp227.storygroup.shared.data.source.NotificationRemoteDataSource
...
class NotificationRepositoryImpl(
    ...
    private val personalEvents: Flow<PersonalEvent> =
        notificationRemoteDataSource.subscribePersonalEvents()
            .mapNotNull { event ->
                when (event) {
    ...
    override fun getNotificationsPagingData(): Flow<PagingData<AppNotification>> =
        Pager(PagePagingConfig) {
            PagePagingSource { page, size ->
                notificationRemoteDataSource.getNotifications(page, size).mapNotNull { it.toDomain() }
            }
        }.flow

    override suspend fun getUnreadCount(): Result<Long> =
        runCatching { notificationRemoteDataSource.getUnreadCount().count }

    override suspend fun markAsRead(notificationId: Long): Result<Unit> =
        runCatching { notificationRemoteDataSource.markAsRead(notificationId) }

    override suspend fun markAllAsRead(): Result<Unit> =
        runCatching { notificationRemoteDataSource.markAllAsRead() }
}
